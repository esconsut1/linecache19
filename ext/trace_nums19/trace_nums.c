...
{
    VALUE         result = rb_ary_new(); /* The returned array of line
                                          * numbers.
                                          */
    int           len;
    char *        token;
    ...
    /* First compile to bytecode, using the method in
     * eval_string_with_cref() in vm_eval.c
     */

    th->parse_in_eval++;
    th->mild_compile_error++;
    iseqval = rb_iseq_compile(src, rb_str_new_cstr("(numbers_for_str)"),
                              INT2FIX(1));
    th->mild_compile_error--;
    th->parse_in_eval--;
    ...
    for (token = strtok(disasm, "\n"); token != NULL;
        ...
        for (; len > 0; len--) {
            ...
            if (token[len] == '(') {
                /* trace found */
                rb_ary_push(result, INT2NUM(atoi(token + len + 1)));
            }
